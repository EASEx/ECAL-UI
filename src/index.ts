...
import { registerHandlers, registerMenu } from './handler';
...
const createWindow = (): void => {
  ...
  let agentView = new BrowserView({ webPreferences: { preload: MAIN_WINDOW_PRELOAD_WEBPACK_ENTRY, } });
  let jupyterView = new BrowserView();
  mainWindow.addBrowserView(agentView);
  mainWindow.addBrowserView(jupyterView);
  agentView.setAutoResize({ width: true, height: true, horizontal: true, vertical: true })
  jupyterView.setAutoResize({ width: true, height: true, horizontal: true, vertical: true })
  agentView.setBounds({ x: 0, y: 0, width: 200, height: 600 });
  jupyterView.setBounds({ x: 200, y: 0, width: 1000, height: 600 });
  ...
  registerMenu(agentView, jupyterView)
};
